import {find, update, remove, create}  from '../models/disaster-model'
...
const getDisasters = async(ctx, next)=> {
    ...
    let result = []
    try {
        result = await find(query).populate('user', 'name age gender phone idNumber address').sort({_id: -1}).skip(offset).limit(pageSize).lean().exec()
        ctx.body = {
            total: count,
            data: result
        }
...
module.exports = {getDisasters, updateDisaster, createDisaster, removeDisaster}
